<?php
/**
 * Postmedia_Sniffs_XSS_EscapeOutputSniff
 * copied from Squiz_Sniffs_XSS_EscapeOutputSniff.
 *
 * PHP version 5
 *
 * @category PHP
 ...
 */
class Postmedia_Sniffs_XSS_EscapeOutputSniff extends WordPress_Sniff
{
	...
	public function process( PHP_CodeSniffer_File $phpcsFile, $stackPtr )
	{
		...
		for ( $i = $stackPtr; $i < $end_of_statement; $i++ ) {
			...
			if ( T_STRING === $this->tokens[ $i ]['code'] ) {
				...
				// Handle functions that are namespaced
				while ( isset( $this->tokens[ $i+1 ] ) && in_array( $this->tokens[ $i+1 ]['code'], array( T_NS_SEPARATOR, T_DOUBLE_COLON ) ) ) {
					$functionName .= $this->tokens[ ++$i ]['content']; // T_NS_SEPARATOR, or T_DOUBLE_COLON
					$functionName .= $this->tokens[ ++$i ]['content']; // Next T_STRING
				}
